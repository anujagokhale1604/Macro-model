...
def load_all_data():
    ...
    # Load FX Data from your new uploads
    ...
    sgd_df = pd.read_csv('AEXSIUS.xlsx - Annual.csv', parse_dates=['observation_date']) # Annual source
    
    return df, inr_df, gbp_df, sgd_df

df_macro, df_inr, df_gbp, df_sgd = load_all_data()

# --- SIDEBAR: NEW FX CONTROLS ---
...
# Fair Value = Neutral + Inflation Gap + (FX Shock * Sensitivity)
...
fig = go.Figure()
# Policy Rate
fig.add_trace(go.Scatter(x=df_macro['Date'], y=df_macro[m['rate']], name="Policy Rate", line=dict(color="#2E5077")))
# FX Rate on Secondary Axis

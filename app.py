...
# --- 1. PREMIUM STYLING (FORCE VISIBILITY) ---
...
st.markdown("""
    ...
    /* Sidebar Styling - Forcing White Text for Visibility */
...
# --- 3. SIDEBAR (ALL TOGGLES RESTORED) ---
with st.sidebar:
...
if df_raw is not None:
    ...
    if show_taylor:
        fig1.add_trace(go.Scatter(x=df['Date'], y=df['Taylor'], name="Taylor Rule Suggestion", line=dict(color='orange', width=2, dash='dash')), secondary_y=False)
    ...
    with colA:
        ...
        verdict = "Policy is <b>restrictive</b>." if lp > lt else "Central Bank is <b>behind the curve</b>."
        st.markdown(f"<div class='note-box'><b>Current Logic:</b> {scenario} with {severity}% severity. <br><br><b>Insight:</b> {verdict}</div>", unsafe_allow_html=True)
    ...
    with colB:
        ...
        st.markdown(f"<div class='note-box'>Synthesis engine for <b>{market}</b> using monthly mean FX resamples and annual GDP step-interpolation.</div>", unsafe_allow_html=True)
